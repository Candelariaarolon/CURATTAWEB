...
export async function POST(req: Request) {
  try {
    ...
    console.log("[webhook] customers/redact recibido:", payload);
  } catch (err) {
    console.error("[webhook] customers/redact: no se pudo parsear el body:", err);
  }

  // Curatta no almacena datos personales de clientes (solo lee catálogo en
  // modo lectura), así que no hay nada que borrar.
